...
// Variabel untuk memantau kawasan_no_pol
$currentKawasan = null;
...
$sql = "SELECT * FROM `tb_kunci_kendaraan` WHERE  
        ...
        ORDER BY kawasan_no_pol ASC, id_vehicle_key ASC";
$result = $koneksi->query($sql);
while ($row = $result->fetch_assoc()) {
    // Cek perubahan kawasan_no_pol
    if ($row['kawasan_no_pol'] != $currentKawasan) {
        // Jika ada perubahan, tambahkan halaman baru
        if ($currentKawasan !== null) {
            $pdf->AddPage();
        }
        // Update nilai kawasan_no_pol saat ini
        $currentKawasan = $row['kawasan_no_pol'];
        
        // Tampilkan header untuk kawasan_no_pol baru
        $pdf->SetFont('Times', 'B', 16);
        $pdf->Cell(59, 1, 'LOG BOOK OPERASIONALKUNCI KENDARAAN PERUSAHAAN', 0, 1, 'C');
        $pdf->Cell(59, 1, "KAWASAN : " . $currentKawasan, 0, 1, 'C');
        $pdf->Cell(59, 1, "HARI/TANGGAL : " . $hariiini . ", ". $date, 0, 1, 'C');
        $pdf->SetFont('Times', '', 13);

        // Tampilkan ulang header kolom
        $pdf->Cell(2,2,'NO',1,0,'C'); 
        $pdf->Cell(5,2,'NO POL','TB',0,'C'); 
        $pdf->Cell(25,1,'DIAMBIL','LTR',0,'C'); 
        $pdf->Cell(25,1,'DIKEMBALIKAN','TR',1,'C'); 
        // second line(row)
        $pdf->Cell(2,1,'',0,0); 
        $pdf->Cell(5,1,'',0,0); 
    ...
    $imagePath = $row['signature_taken']; // Adjust path as needed
    if (file_exists($imagePath)) {
        $pdf->Cell(3, 1, $pdf->Image($imagePath, $pdf->GetX(), $pdf->GetY(), 2, 0.8), 1, 0, 'L', false);
    }
